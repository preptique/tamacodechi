...
describe('responses', () => {
  test('companion_status returns 3+ variants', () => {
    ...
    for (let i = 0; i < 100; i++) {
      variants.add(companionStatus('Gravy', 'duck', MOCK_MOOD, MOCK_MOOD_LABEL, MOCK_TOTAL_FEEDS, MOCK_TOTAL_PETS))
    }
    assert(variants.size >= 3, `status has ${variants.size} variants, want 3+`)
  })

  test('companion_status includes name', () => {
    const r = companionStatus('Butters', 'cat', MOCK_MOOD, MOCK_MOOD_LABEL, 0, 0)
    assert(r.includes('Butters'), `includes name, got: ${r}`)
  })

  test('companion_status includes mood', () => {
    const r = companionStatus('Gravy', 'duck', 85, 'happy', 10, 3)
  ...
  test('companion_feed returns 3+ variants', () => {
    const variants = new Set<string>()
    for (let i = 0; i < 100; i++) {
      variants.add(companionFeed('Gravy', 'duck', 'const x = 1', MOCK_MOOD, MOCK_MOOD_LABEL, MOCK_TOTAL_FEEDS))
    }
    assert(variants.size >= 3, `feed has ${variants.size} variants, want 3+`)
  })

  test('companion_feed handles empty code string', () => {
    const r = companionFeed('Gravy', 'duck', '', MOCK_MOOD, MOCK_MOOD_LABEL, 0)
  ...
  test('companion_feed counts code length correctly', () => {
    const short = companionFeed('Gravy', 'duck', 'x'.repeat(50), MOCK_MOOD, MOCK_MOOD_LABEL, 0)
    const medium = companionFeed('Gravy', 'duck', 'x'.repeat(250), MOCK_MOOD, MOCK_MOOD_LABEL, 0)
    assert(short !== medium, `different lengths produce different responses`)
  })

  test('companion_feed includes name', () => {
    const r = companionFeed('Bubbles', 'goose', 'hello world', MOCK_MOOD, MOCK_MOOD_LABEL, 0)
  ...
  test('companion_pet returns 3+ variants', () => {
    const variants = new Set<string>()
    for (let i = 0; i < 100; i++) {
      variants.add(companionPet('Gravy', 'duck', MOCK_MOOD, MOCK_MOOD_LABEL, MOCK_TOTAL_PETS))
    }
    assert(variants.size >= 3, `pet has ${variants.size} variants, want 3+`)
  })

  test('companion_pet includes name', () => {
    const r = companionPet('Snuggles', 'cat', MOCK_MOOD, MOCK_MOOD_LABEL, 0)
    assert(r.includes('Snuggles'), `includes name, got: ${r}`)
  })

  test('companion_pet includes mood label', () => {
    const r = companionPet('Gravy', 'duck', 85, 'happy', 0)
